...
if(is_object($db) && class_exists(orderedtable)){
    if(!DISTINGUISH_BY_HASH){
        ...
        $query	= 'SELECT id, hash, handle, (SELECT elo FROM players WHERE hash = p.hash AND elo IS NOT NULL ORDER BY gid DESC LIMIT 1 ) AS elo FROM profiles AS p ORDER BY elo DESC';    
    }
